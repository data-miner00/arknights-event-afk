...
def from_login_to_lobby():
    ...
    while True:
        facebook_login_btn = try_locate_image_on_screen(LoginScreen1.FACEBOOK_LOGIN_BUTTON.value)
        enter_game_btn = try_locate_image_on_screen(LoginScreen2.ENTER_GAME_BUTTON.value)
    ...
    if facebook_login_btn:
        ...
        wait_until_operation_completed(
            lambda: locate_image_position_and_click(LoginScreen1.FACEBOOK_LOGIN_BUTTON.value)
        )
        wait_for_seconds(3)
...
def close_calendar():
    ...
    wait_until_operation_completed(
        lambda: locate_image_position_and_click(HomeScreen.DAILY_REWARD_EXIT_BUTTON.value)
    )
